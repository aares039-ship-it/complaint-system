...
<head>
    ...
    <title>Modify the complaint | Dashboard</title>
    ...
    <div class="container-sm">
        <div class="card">
            <div class="card-header">
                <h2>Modify the complaint #<?php echo $complaint_id; ?></h2>
                <p>Update the informations or change the status of the request.</p>
            </div>
            ...
            <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'] . '?id=' . $complaint_id); ?>" method="post" class="styled-form">
                ...
                <div class="admin-action-box">
                    <div class="form-group <?php echo (!empty($status_err)) ? 'has-error' : ''; ?>">
                        <label for="status">Change statut of the complaint</label>
                        <select name="status" id="status" class="status-select">
                            <option value="pending" <?php if($status==='pending') echo 'selected'; ?>>🟡 in attente (Pending)</option>
                            <option value="resolved" <?php if($status==='resolved') echo 'selected'; ?>>🟢 Resolute (Resolved)</option>
                        </select>
